...
$i = 0;
while ($row = mysqli_fetch_array($result)) {	$id = $row['P_ID'];
$qty = $row['P_Quantity'];
$name = $row['P_Name'];
$dataArray[$i] = $qty;
$nameArray[$i] = $name;
...
array("y" => $dataArray[10], "label" =>  $nameArray[10]),
);
?>
<!doctype html>
<html lang="en">
...
title: {
text: "Stock Reserves"};
axisY: {title: "Biscuits", interval: 10, minimum: 0,},
data: [{type: "column", yValueFormatString: "# units", dataPoints: <?php echo json_encode($dataPoints, JSON_NUMERIC_CHECK); ?>
}]
});
chart.render();
}
...
</div><script src="https://canvasjs.com/assets/script/canvasjs.min.js"></script>
